...
const CryptoJoe = artifacts.require("./CryptoJoeContract.sol");

require("chai")
  .use(require("chai-as-promised"))
  .should();

contract("CryptoJoe", async (accounts) => {
  let cryptoJoes, result, cryptoJoeCount;

  before(async () => {
    cryptoJoes = await CryptoJoe.deployed();
  });

  describe("Deployment", async () => {
    it("contract has an address", async () => {
      const address = await cryptoJoes.address;
    ...
    it("has a name", async () => {
      const name = await cryptoJoes.collectionName();
      assert.equal(name, "CryptoJoe Collection");
    });

    it("has a symbol", async () => {
      const symbol = await cryptoJoes.collectionNameSymbol();
  ...
  describe("application features", async () => {
    it("allows users to mint ERC721 token", async () => {
      cryptoJoeCount = await cryptoJoes.cryptoJoeCounter();
      assert.equal(cryptoJoeCount.toNumber(), 0);

      let tokenExists;
      tokenExists = await cryptoJoes.getTokenExists(1, { from: accounts[0] });
      assert.equal(tokenExists, false);

      let tokenURIExists;
      tokenURIExists = await cryptoJoes.tokenURIExists(
      ...
      let tokenNameExists;
      tokenNameExists = await cryptoJoes.tokenNameExists("myCJNFT", {
        from: accounts[0],
      });
      ...
      for (let i = 0; i < colorsArray1.length; i++) {
        colorExists = await cryptoJoes.colorExists(colorsArray1[i], {
          from: accounts[0],
        });
        assert.equal(colorExists, false);
      }

      result = await cryptoJoes.mintCryptoJoe(
        "myCJNFT",
        "https://gateway.pinata.cloud/ipfs/QmYFmJgQGH4uPHRYN15Xdv4aLd9o4Aq63y1e4GgN6kj5aK/2",
      ...
      cryptoJoeCount = await cryptoJoes.cryptoJoeCounter();
      assert.equal(cryptoJoeCount.toNumber(), 1);
      ...
      let cryptojoe;
      cryptojoe = await cryptoJoes.allCryptoJoes(1, {
        from: accounts[0],
      });
      assert.equal(cryptojoe.tokenId.toNumber(), 1);
      assert.equal(cryptojoe.tokenName, "myCJNFT");
      assert.equal(
        cryptojoe.tokenURI,
        "https://gateway.pinata.cloud/ipfs/QmYFmJgQGH4uPHRYN15Xdv4aLd9o4Aq63y1e4GgN6kj5aK/2"
      );
      assert.equal(cryptojoe.mintedBy, accounts[0]);
      assert.equal(cryptojoe.currentOwner, accounts[0]);
      assert.equal(
        cryptojoe.previousOwner,
        0x0000000000000000000000000000000000000000
      );
      assert.equal(web3.utils.fromWei(cryptojoe.price, "ether"), 1);
      assert.equal(cryptojoe.numberOfTransfers.toNumber(), 0);
      assert.equal(cryptojoe.forSale, true);
      ...
      await cryptoJoes.mintCryptoJoe(
        "myCJNFT2",
    ...
    it("returns address of the token's owner", async () => {
      const tokenOwner = await cryptoJoes.getTokenOwner(2);
      assert.equal(tokenOwner, accounts[1]);
    });

    // returns tokenURI of the token
    it("returns metadata of a token", async () => {
      const tokenMetaData = await cryptoJoes.getTokenMetaData(2);
      assert.equal(
        tokenMetaData,
        "https://gateway.pinata.cloud/ipfs/QmYFmJgQGH4uPQRYN15Xdv4aLd9o4Aq63y1e4GgN6kj5aK/2"
      );
    ...
    it("returns total number of tokens minted so far", async () => {
      const totalNumberOfTokensMinted = await cryptoJoes.getNumberOfTokensMinted();
      assert.equal(totalNumberOfTokensMinted.toNumber(), 4);
    });

    it("returns total number of tokens owned by an address", async () => {
      const totalNumberOfTokensOwnedByAnAddress = await cryptoJoes.getTotalNumberOfTokensOwnedByAnAddress(
        accounts[0]
      );
    ...
    it("allows users to buy token for specified ethers", async () => {
      const oldTokenOwner = await cryptoJoes.getTokenOwner(1);
      ...
      oldTotalNumberOfTokensOwnedBySeller = await cryptoJoes.getTotalNumberOfTokensOwnedByAnAddress(
        accounts[0]
      );
      assert.equal(oldTotalNumberOfTokensOwnedBySeller.toNumber(), 3);

      let cryptoBoy;
      cryptoBoy = await cryptoJoes.allCryptoJoes(1, {
        from: accounts[0],
      });
      assert.equal(cryptoBoy.numberOfTransfers.toNumber(), 0);

      result = await cryptoJoes.buyToken(1, {
        from: accounts[2],
        value: web3.utils.toWei("1", "Ether"),
      });

      const newTokenOwner = await cryptoJoes.getTokenOwner(1);
      ...
      newTotalNumberOfTokensOwnedBySeller = await cryptoJoes.getTotalNumberOfTokensOwnedByAnAddress(
        accounts[0]
      );
      ...
      await cryptoJoes.buyToken(2, {
        from: 0x0000000000000000000000000000000000000000,
        value: web3.utils.toWei("1", "Ether"),
      }).should.be.rejected;

      await cryptoJoes.buyToken(56, {
        from: accounts[4],
        value: web3.utils.toWei("1", "Ether"),
      }).should.be.rejected;

      await cryptoJoes.buyToken(3, {
        from: accounts[0],
        value: web3.utils.toWei("1", "Ether"),
      }).should.be.rejected;
    });
    ...
    it("allows users to change token price", async () => {
      let cryptoBoyPrice;
      cryptoBoyPrice = await cryptoJoes.allCryptoJoes(1, {
        from: accounts[0],
      });
      assert.equal(web3.utils.fromWei(cryptoBoyPrice.price, "ether"), 1);

      result = await cryptoJoes.changeTokenPrice(
        1,
      ...
      await cryptoJoes.changeTokenPrice(1, web3.utils.toWei("3", "Ether"), {
        from: 0x0000000000000000000000000000000000000000,
      }).should.be.rejected;

      await cryptoJoes.changeTokenPrice(82, web3.utils.toWei("3", "Ether"), {
        from: accounts[2],
      }).should.be.rejected;
    ...
    it("allows users to toggle between setting the token for sale or not for sale", async () => {
      ...
      result = await cryptoJoes.toggleForSale(1, { from: accounts[2] });

      cryptojoe = await cryptoJoes.allCryptoJoes(1, {
        from: accounts[0],
      });
      assert.equal(cryptojoe.forSale, false);
      ...
      await cryptoJoes.toggleForSale(1, {
        from: 0x0000000000000000000000000000000000000000,
      }).should.be.rejected;

      await cryptoJoes.toggleForSale(94, { from: accounts[2] }).should.be
        .rejected;

      await cryptoJoes.toggleForSale(1, { from: accounts[8] }).should.be
        .rejected;
    });
  });
});
